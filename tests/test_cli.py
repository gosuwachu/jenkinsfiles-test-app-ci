...
class TestCliParsing:
    ...
    def test_ios_build(self, mock_run):
        with patch(
            "sys.argv",
            ["ci-cli", "ios", "build", "--commit-sha", "abc", "--gh-token", "tok", "--build-url", "http://b"],
        ):
            main()
    ...
    def test_android_linter(self, mock_run):
        with patch(
            "sys.argv",
            ["ci-cli", "android", "linter", "--commit-sha", "def", "--gh-token", "tok", "--build-url", "http://b"],
        ):
            main()
    ...
    def test_ios_ui_tests_with_pr(self, mock_run):
        with patch(
            "sys.argv",
            ["ci-cli", "ios", "ui-tests", "--pr-number", "42", "--comment-author", "user", "--gh-token", "tok", "--build-url", "http://b"],
        ):
            main()
